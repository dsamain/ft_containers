...
/*   Updated: 2022/06/15 02:57:25 by dsamain          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft {
	...
	/*------------------Member type------------------*/
		...
		public:
			...
			value_compare(const key_compare &c = Compare()) : comp(c) {}//std::cout << "normal called" << std::endl;}
			value_compare(const value_compare &cpy) : comp(cpy.comp) {}//std::cout << "called" << std::endl;}
		protected:
			Compare comp;
		};

		typedef bidirectional_iterator<value_type, value_compare, Alloc, node<value_type>, 0> iterator;
		typedef bidirectional_iterator<value_type, value_compare, Alloc, node<value_type>, 1> const_iterator;
	...
	/*------------------Constructor------------------*/
		...
		: _alloc(alloc), _keyComp(comp), _comp(comp), tree(value_compare(comp)) {
		}		
	   ...
	   : _alloc(alloc), _keyComp(comp), _comp(comp), tree(value_compare(comp)) {
		...
		map(const map &cpy)
		: _alloc(cpy._alloc), _keyComp(cpy._keyComp), _comp(cpy._comp), tree(value_compare(cpy._comp)) {
